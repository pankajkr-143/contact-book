...
class PhoneBook:
    ...
    def save_contact_to_db(self):
        ...
        if (
            ...
            or self.check_int(PhoneNo) == False
            or self.check_int(Address) == False
        ):
            result = tkMessageBox.showwarning(
            ...
            if len(res) == 0:
                self.cursor.execute(
                    f"""INSERT INTO CONTACTS_TABLE VALUES(
                                '{name}','{mobile}','{PhoneNo}','{Address}','{email}','{notes}')
                                """
                )
                self.con.commit()
    ...
    def main_page_gui(self):
        ...
        self.tree.heading("3", text="PhoneNo")
    ...
    def edit_contact_gui(self, contact_data):
        ...
        self.PhoneNo.set(contact_data[2])
        self.Address = tk.StringVar()
        self.Address.set(contact_data[3])
...
def load_dataset():
    ...
    if not fetch:
        ...
        ("Macky's Tech","8235910315",12345678,12345678,"mackystech@example.com","A Test Contact Card."),
        ("Macky's Tech","823590315",12345678,12345678,"nnn@example.com","A Test Contact Card.")
        """
        )
    return con
